...
export default function OrcidConnect({
  ...
  const t = useTranslations("orcidConnect");
  const locale = useLocale();

  const banner = redirectStatus && STATUS_TONE[redirectStatus]
    ? { tone: STATUS_TONE[redirectStatus], text: t(`status.${redirectStatus}`) }
    : null;
      ...
      {status.orcidOauthVerifiedAt && status.orcid ? (
        <div className="flex flex-col gap-1.5 rounded-lg border border-green-200 bg-green-50 p-3">
          ...
          <p className="text-[11px] text-green-600">
            {t("verifiedAtPrefix")} {new Date(status.orcidOauthVerifiedAt).toLocaleString(locale)}
          </p>
        </div>
      ) : (
        <>
          {status.orcid && (
            <p className="text-xs text-gray-500">
              {t("existingOrcidNotice", {
                orcid: status.orcid,
                verificationStatus: status.orcidVerifiedAt
                  ? t("verificationStatusManual")
                  : t("verificationStatusUnverified"),
              })}
            </p>
          )}
          ...
          {!isConfigured ? (
            <div className="flex items-start gap-2 rounded-lg bg-amber-50 p-3 text-xs text-amber-700">
              <Info className="mt-0.5 h-3.5 w-3.5 shrink-0" />
              {t.rich("notConfiguredNotice", {
                clientId: (chunks) => (
                  <code className="mx-1 rounded bg-amber-100 px-1 py-0.5">{chunks}</code>
                ),
                clientSecret: (chunks) => (
            ...
            <form action={formAction}>
              ...
              >
                ...
                {pending ? t("connectingButton") : t("connectButton")}
              </button>
            </form>
          )}
